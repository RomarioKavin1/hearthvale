...
**Testing touch scroll on mobile.** Two surfaces are easy to regress and hard to catch on a desktop mouse, so test both under touch emulation (Chrome DevTools device toolbar, or a real phone via the Reddit app):

- **In-game sheets** — open a tall modal (Market, Level unlocks, Leaderboards) on a short viewport and drag the body up/down. The `.hv-modal-body` must scroll under a finger. The two guards that make it work are `min-height: 0` (so the flex body can shrink and own its overflow instead of spilling out of the modal) and `touch-action: pan-y`. Phaser sets `touch-action: none` only on `#game-container` (the canvas host), never on `#app`/`body`/`html`, so it can't swallow sheet gestures.
- **Inline splash** — the feed post must never trap the redditor's scroll. `src/client/splash.{ts,css}` install **no** `touchmove`/`preventDefault` handlers, the CTA is a plain `click`, and `#splash-root` carries `touch-action: pan-y`, so a swipe over the post keeps scrolling the feed.
